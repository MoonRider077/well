...
import { removeFromCart, removeAllFromCart, increase, decrease } from '../../../redux/cartSlice';
...
const Cart: React.FC<CartProps> = ({ isVisible, onClose }) => {
  ...
  const totalPrice = cartItems.reduce((total: number, item: CartItem) => total + parseFloat(item.price.replace(/\s/g, '')) * item.quantity, 0);
  ...
  return (
    <div className={`z-[9999] w-[45%] sm:w-full rounded-tl-[16px] rounded-bl-[16px] h-screen fixed bg-white top-0 right-0 transition-transform duration-300 ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className='p-10 sm:p-5'>
          ...
          <h3 className='mt-5 font-medium text-[#677E8B] cursor-pointer' onClick={handleRemoveAllFromCart}>
            {t('cart.resetCart')}
          </h3>
        </div>
          <div className='px-10 sm:px-5 overflow-y-auto' style={{ maxHeight: 'calc(100vh - 300px)' }}>
          {cartItems.length === 0 ? (
                        ...
                        <div key={item.id} className={`hidden sm:block ${index !== 0 ? 'mt-10' : ''}`}>
                          <div className='flex relative'>
                            <div className='border border-solid border-[#F0F2F6] rounded-[16px]'>
                              <img src={item.image} alt="product" className='h-auto max-w-24' />
                            </div>
        ...
        <footer className='absolute bg-[#F7F8FA] bottom-0 flex sm:block justify-between sm:p-5 py-6 px-10 w-full'>
          <div className='sm:flex sm:justify-between'>
            <p>{t('cart.total')}</p>
            <p>{totalPrice} {t(cartItems.length > 0 ? cartItems[0].curKey : '')}</p> 
          </div>
